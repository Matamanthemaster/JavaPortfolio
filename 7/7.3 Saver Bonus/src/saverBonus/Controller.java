...
public class Controller
{
    ...
    void btnApplyBonusPressed()
    {
        ...
        if (flagPercentBonusValid)
        {
            ...
            {
                //if negative we don't want to take money.
                if (arrSaverTabs[i].balance > 0)
                {
                    //convert percentage to decimal, add 1 to incorporate base value so we can multiply to get value after bonus.
                    arrSaverTabs[i].balance *= 1 + (Double.valueOf(bonusPercentage)/100);

                    //prevent balance from going higher than max (% is positive only).
                    if (arrSaverTabs[i].balance > Saver.maxBalance)
                    {
                        arrSaverTabs[i].balance = Saver.maxBalance;
                    }

                    //update display
                    arrSaverTabs[i].DisplayBalance();
                }
            }
        }
    }
